...
### Test cases for AddStudent API :
    ...
    def test_add_student_with_valid_payload_after_login_by_admin_credentials(self):
        login = self.client.post(reverse('login'), data=json.dumps(self.admin_login_payload), content_type=CONTENT_TYPE)
    ...
    def test_add_student_with_valid_payload_after_login_by_mentor_credentials(self):
        login = self.client.post(reverse('login'), data=json.dumps(self.mentor_login_payload), content_type=CONTENT_TYPE)
        token = login['authorization']
        auth_headers = {
            'HTTP_AUTHORIZATION': token,
        }
    ...
    def test_add_student_with_valid_payload_after_login_by_student_credentials(self):
        login = self.client.post(reverse('login'), data=json.dumps(self.student_login_payload), content_type=CONTENT_TYPE)
        token = login['authorization']
        auth_headers = {
            'HTTP_AUTHORIZATION': token,
        }
